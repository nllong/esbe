...
class UOCliWrapper(object):
    ...
    def _run_command(self, command):
        ...
        try:
            ...
            with open(self.log_file, 'a') as log:
                ...
                # These env vars come directly from the ~/.env_uo.sh file. Update if a new version is installed.
                new_env['GEM_HOME'] = f"/Applications/URBANoptCLI_{self.uo_version}/gems/ruby/2.7.0"
                new_env['GEM_PATH'] = f"/Applications/URBANoptCLI_{self.uo_version}/gems/ruby/2.7.0"
                new_env['PATH'] = f"/Applications/URBANoptCLI_{self.uo_version}/ruby/bin:/Applications/URBANoptCLI_{self.uo_version}/gems/ruby/2.7.0/bin:{os.environ['PATH']}"
                new_env['RUBYLIB'] = f"/Applications/URBANoptCLI_{self.uo_version}/OpenStudio/Ruby"
                new_env['RUBY_DLL_PATH'] = f'/Applications/URBANoptCLI_{self.uo_version}/OpenStudio/Ruby'
                new_env['GEM_DEVELOPER_KEY'] = os.environ['GEM_DEVELOPER_KEY']
                result = subprocess.run(command, stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True, env=new_env)
    ...
    def process_reopt_scenario(self, feature_file, scenario_name, individual_features=False):
        # In UO, the -r flag is used for the aggregrated load analysis, whereas
