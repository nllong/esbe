...
module URBANopt
  module Scenario
    class ResidentialMapper < BaselineMapper
      def create_osw(scenario, features, feature_names)
        ...
        if residential_building_types.include? building_type
          ...
          args[:wall_assembly_r] = Float(args[:wall_assembly_r]) * 1.5 # 50% increase
          # the following are no longer required in HPXML v1.5.0
          # if this isn't set, set to 90% (0.9), else reduce to 90% of set value
          args[:misc_plug_loads_television_usage_multiplier] = args[:misc_plug_loads_television_usage_multiplier].nil? ? 0.9 : Float(args[:misc_plug_loads_television_usage_multiplier]) * 0.75 # 25% reduction
          args[:misc_plug_loads_other_usage_multiplier] = args[:misc_plug_loads_other_usage_multiplier].nil? ? 0.9 : Float(args[:misc_plug_loads_other_usage_multiplier]) * 0.75 # 25% reduction
          args[:lighting_interior_usage_multiplier] = args[:lighting_interior_usage_multiplier].nil? ? 0.9 : Float(args[:lighting_interior_usage_multiplier]) * 0.6 # 40% reduction
          args[:lighting_exterior_usage_multiplier] = args[:lighting_exterior_usage_multiplier].nil? ? 0.9 : Float(args[:lighting_exterior_usage_multiplier]) * 0.6 # 40% reduction
          ...
          args[:clothes_washer_usage_multiplier] = args[:clothes_washer_usage_multiplier].nil? ? 0.9 : Float(args[:clothes_washer_usage_multiplier]) * 0.7 # 30% reduction
          args[:clothes_dryer_usage_multiplier] = args[:clothes_dryer_usage_multiplier].nil? ? 0.9 : Float(args[:clothes_dryer_usage_multiplier]) * 0.8 # 20% reduction
          ...
          args[:water_fixtures_usage_multiplier] = args[:water_fixtures_usage_multiplier].nil? ? 0.9 : Float(args[:water_fixtures_usage_multiplier]) * 0.7 # 30% reduction
          ...
          OpenStudio::Extension.set_measure_argument(osw, 'ReduceElectricEquipmentLoadsByPercentage', 'elecequip_power_reduction_percent', 30)

          OpenStudio::Extension.set_measure_argument(osw, 'ReduceLightingLoadsByPercentage', '__SKIP__', false)
          OpenStudio::Extension.set_measure_argument(osw, 'ReduceLightingLoadsByPercentage', 'lighting_power_reduction_percent', 40)
        end
